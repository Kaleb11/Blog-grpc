...
func (ac *AuthController) SignUpUser(ctx *gin.Context) {
	...
	err = utils.SendMail(newUser, &emailData, "verificationCode.html")
	if err != nil {
...
func (ac *AuthController) SignInUser(ctx *gin.Context) {
	...
	refresh_token, err := utils.CreateToken(config.RefreshTokenExpiresIn, user.ID, config.AccessTokenPrivateKey)
	if err != nil {
		ctx.JSON(http.StatusBadRequest, gin.H{"status": "fail", "message": err.Error()})
		return
	}
...
func (ac *AuthController) ForgotPassword(ctx *gin.Context) {
	...
	fmt.Println("This user is", user.Email, err)
	if err != nil {
		if err == mongo.ErrNoDocuments {
			ctx.JSON(http.StatusBadRequest, gin.H{"status": "fail", "message": "The email address is not valid"})
			return
		}
		ctx.JSON(http.StatusBadGateway, gin.H{"status": "error", "message": err.Error()})
		return
	...
	err = utils.SendMail(user, &emailData, "resetPassword.html")
	if err != nil {
		ctx.JSON(http.StatusBadGateway, gin.H{"status": "success", "message": "There was an error sending email"})
		return
	}
